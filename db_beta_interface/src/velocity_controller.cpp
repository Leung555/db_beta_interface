...
bool db_beta_interface::initControlItems(){
  ...
  bool result = true;
	...
	const ControlItem* goal_velocity = dxl_wb->getItemInfo(dxl_id, "Goal_Velocity");
...
void db_beta_interface::initSyncReadWriteHandler(){
  ...
  // uint16_t write_length = control_items["Goal_Position"]->data_length;
	uint16_t write_start_adress = control_items["Goal_Velocity"]->address;
  uint16_t write_length = control_items["Goal_Velocity"]->data_length;
...
void db_beta_interface::writeDxl_subscribe_callback(const Jointstate & msg)
{
  ...
  for(int dxl_cnt = 0; dxl_cnt< goal_motor_cnt_msg; dxl_cnt++){
    ...
    goal_velocity[dxl_cnt] = dxl_wb->convertVelocity2Value(goal_id_array_uint8[dxl_cnt], msg.velocity[dxl_cnt]);    
