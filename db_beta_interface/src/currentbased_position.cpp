...
bool db_beta_interface::initControlItems(){
  ...
  const ControlItem* goal_current = dxl_wb->getItemInfo(dxl_id, "Goal_Current");
...
void db_beta_interface::initSyncReadWriteHandler(){
  ...
  uint16_t write_start_adress_cur = control_items["Goal_Current"]->address;
  uint16_t write_length_cur = control_items["Goal_Current"]->data_length;
  printf("write_start_adress_cur: %d  \n", write_start_adress_cur);
  printf("write_length: %d  \n", write_length_cur);

  result = dxl_wb->addSyncWriteHandler(write_start_adress_cur, write_length_cur, &log);
  if (result == false)
  {
    printf("%s\n", log);
    printf("Failed to add sync write handler\n");
...
void db_beta_interface::setupDynamixel()
{
  ...
  for(int dxl_cnt=0; dxl_cnt < motor_cnt; dxl_cnt++){
    ...
    result = dxl_wb->setCurrentBasedPositionControlMode( id_array[dxl_cnt], &log);
    if (result == false) {
      RCLCPP_ERROR(rclcpp::get_logger("db_beta_interface"), "Failed to set Current Based Position Control Mode id: %d.", id_array[dxl_cnt]);
    } else {
      RCLCPP_INFO(rclcpp::get_logger("db_beta_interface"), "Succeeded to set Current Based Position Control Mode id: %d.", id_array[dxl_cnt]);
    }
  }
  if (TorqueOn) 
  {
...
void db_beta_interface::writeDxl_subscribe_callback(const Jointstate & msg)
{
  ...
  uint8_t goal_motor_cnt_msg = msg.position.size();
    ...
    for(int dxl_cnt = 0; dxl_cnt < goal_motor_cnt_msg; dxl_cnt++) {
        // if (motorNameIDPair.find(msg.name[dxl_cnt]) == motorNameIDPair.end()) {
        //     std::cerr << "Error: Motor name not found in motorNameIDPair: " << msg.name[dxl_cnt] << std::endl;
        //     continue; // or return; if you want to abort the whole callback
        // }
  ...
  printf("Success to sync write position\n");

//   write multiple goal current to motors
//   uint8_t goal_motor_cnt_msg = msg.effort.size();
//   uint8_t goal_id_array_uint8[goal_motor_cnt_msg];
  int32_t goal_current[goal_motor_cnt_msg];
  cout << "goal_motor_cnt_msg: " << +goal_motor_cnt_msg << endl;
  
  for(int dxl_cnt = 0; dxl_cnt < goal_motor_cnt_msg; dxl_cnt++){
      goal_id_array_uint8[dxl_cnt] = (uint8_t) motorNameIDPair[msg.name[dxl_cnt]];
      // goal_current[dxl_cnt] = dxl_wb->convertCurrent2Value(msg.effort[dxl_cnt]);
      goal_current[dxl_cnt] = msg.effort[dxl_cnt];
      cout << "goal_id_array_uint8[dxl_cnt]_current: " << +dxl_cnt 
               << "  goal_current[dxl_cnt]: " << +goal_current[dxl_cnt] << endl;
  }
  
  // Use handler_index 1 for Goal_Current syncWrite (assuming handler_index 1 is for Goal_Current)
  result = dxl_wb->syncWrite(1, 
                                  goal_id_array_uint8, 
                                  goal_motor_cnt_msg, 
                                  goal_current, 
                                  1, 
